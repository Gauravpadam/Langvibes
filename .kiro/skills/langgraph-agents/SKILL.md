...
- Model termination in state — `recursion_limit` is a backstop only
...
result = graph.invoke(
    {"messages": [("human", "Hello")], "iterations": 0, "status": "running"},
    config=config,
)
...
Irreversible actions must be behind `interrupt()`. Requires a checkpointer.
...
- `interrupt()` without a checkpointer — it won't suspend
