...
argument-hint: "[task e.g. 'RAG pipeline over PDF with Chroma']"
user-invocable: true
...
```python
# Similarity (default k=4 is often too few — use 6-10)
retriever = vectorstore.as_retriever(search_type="similarity", search_kwargs={"k": 6})

# MMR — diverse results
retriever = vectorstore.as_retriever(search_type="mmr", search_kwargs={"k": 6, "fetch_k": 20})

# Multi-query — improves recall with query variants
...
Make the retriever a `@tool` and wire into a LangGraph agent — never use `ConversationalRetrievalChain`.
...
- Chunk size exceeding embedding model limit — Titan v2: 8192 tokens, Cohere: 512 tokens
- No chunk overlap — use 10-20% of chunk size
- Default `k=4` too low — use 6-10 for complex queries
- Using deprecated chains — `RetrievalQA` and `ConversationalRetrievalChain` are removed
